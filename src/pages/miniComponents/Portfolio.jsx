...
const Portfolio = () => {
  ...
  useEffect(() => {
    const getMyProjects = async () => {
      try {
        const { data } = await api.get("/project/getall");
    ...
    return (
      <div>
        <div className="relative mb-12">
          <div className="bg-slate-200 dark:bg-slate-800 animate-pulse rounded h-12 w-48 mx-auto"></div>
          <span className="absolute w-full h-1 top-7 sm:top-7 md:top-8 lg:top-11 z-[-1] bg-slate-200"></span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="bg-slate-200 dark:bg-slate-800 animate-pulse rounded aspect-video"></div>
          ))}
        </div>
      </div>
    );
